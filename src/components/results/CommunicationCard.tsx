...
export function CommunicationCard({ card }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (cardRef.current) {
        await exportCardAsPng(cardRef.current, `somabridge-card-${card.emotion.toLowerCase()}.png`);
      }
        ...
        {isFullscreen && (
          ...
          >
            <div className={styles.fullscreenHeader}>
              <span className={styles.fullscreenLabel}>SOMABRIDGE HOLD-UP MODE</span>
              <button

import { getLocalDB, type SomaBridgeDB } from './schema';

// ── Check-in operations ──

export async function saveCheckinLocal(
  checkin: Omit<SomaBridgeDB['checkins']['value'], 'synced' | 'synced_at'>
) {
...
export async function upsertDictionaryLocal(
  entry: SomaBridgeDB['dictionary']['value']
) {
  const db = await getLocalDB();
...
export async function saveCopingLocal(
  entry: Omit<SomaBridgeDB['copingLog']['value'], 'synced' | 'synced_at'>
) {
  const db = await getLocalDB();
...
export async function saveCardLocal(
  card: Omit<SomaBridgeDB['cards']['value'], 'synced' | 'synced_at'>
) {
  const db = await getLocalDB();
  await db.put('cards', {
    ...card,

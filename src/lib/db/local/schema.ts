...
export interface SomaBridgeDB extends DBSchema {
  checkins: {
    key: string;
    value: {
      id: string;
...
let dbPromise: Promise<IDBPDatabase<SomaBridgeDB>> | null = null;

export function getLocalDB(): Promise<IDBPDatabase<SomaBridgeDB>> {
  if (typeof window === 'undefined') {
    throw new Error('IndexedDB is only available in the browser');
  }
  
  if (!dbPromise) {
    dbPromise = openDB<SomaBridgeDB>('somabridge-local', 1, {
      upgrade(db) {
        // Checkins store
